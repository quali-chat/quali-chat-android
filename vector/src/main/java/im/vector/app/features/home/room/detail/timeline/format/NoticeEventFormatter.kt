...
class NoticeEventFormatter @Inject constructor(
    ...
    private fun formatRoomNameEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.name.isNullOrBlank()) {
            ...
            if (event.isSentByCurrentUser()) {
                sp.getString(CommonStrings.notice_room_name_changed_by_you, content.name.removeTgTag())
            } else {
                sp.getString(CommonStrings.notice_room_name_changed, senderName, content.name.removeTgTag())
            }
        }
    }
