This document aims to describe how quali.chat android displays notifications to the end user. It also clarifies notifications and background settings in the app.
...
* [quali.chat Notification implementations](#quali.chat-notification-implementations)
...
When the quali.chat Android app is open (i.e in foreground state), the default timeout is 30 seconds, and delay is 0.
...
That means that quali.chat Android, a matrix client created by New Vector, is using a **Push Gateway** with the needed credentials (FCM API secret Key) in order to send push to the New Vector client.
...
Notifications have 2 'levels' (`highlighted = true/false sound = default/custom`). In quali.chat these notifications level are reflected as Noisy/Silent. 
...
For an application like quali.chat, where users can receive important information at anytime, the best option is to rely on a push system (Google's Firebase Message a.k.a FCM). FCM high priority push can wake up the device and whitelist an application to perform background task (for a limited but unspecified amount of time). 
...
## quali.chat Notification implementations

### Requirements

quali.chat Android must work with and without FCM.
* The quali.chat android app published on F-Droid do not rely on FCM (all related dependencies are not present)
* The quali.chat android app published on google play rely on FCM, with a fallback mode when FCM registration has failed (e.g outdated or missing Google Play Services)

### Foreground sync mode (Gplay and F-Droid)

When in foreground, quali.chat performs sync continuously with a timeout value set to 10 seconds (see HttpPooling).

As this mode does not need to live beyond the scope of the application, and as per Google recommendation, quali.chat uses the internal app resources (Thread and Timers) to perform the syncs. 

This mode is turned on when the app enters foreground, and off when enters background.

In background, and depending on whether push is available or not, quali.chat will use different methods to perform the syncs (Workers / Alarms / Service)

### Push (FCM) received in background 

In order to enable Push, quali.chat must first get a push token from the firebase SDK, then register a pusher with this token on the homeserver.

When a message should be notified to a user,  the user's homeserver notifies the registered `push gateway` for quali.chat, that is [sygnal](https://github.com/matrix-org/sygnal) _- The reference implementation for push gateways -_ hosted by matrix.org. 

This sygnal instance is configured with the required FCM API authentication token, and will then use the FCM API in order to notify the user's device running quali.chat.

```
Homeserver ----> Sygnal (configured for quali.chat) ----> FCM ----> quali.chat
```
...
quali.chat needs then to synchronise with the user's homeserver, in order to resolve the event and create a notification.

As per [Google recommendation](https://android-developers.googleblog.com/2018/09/notifying-your-users-with-fcm.html), quali.chat will then use the WorkManager API in order to trigger a background sync. 
...
```
Homeserver ----> Sygnal ----> FCM ----> quali.chat
                                        (Sync) ----> Homeserver
                                               <----
...
Upon reception of the FCM push, quali.chat will perform a sync call to the homeserver, during this process it is possible that:
...
quali.chat implements several strategies in these cases (TODO document)

### FCM Fallback mode

It is possible that quali.chat is not able to get a FCM push token.
...
If quali.chat is able to detect one of this cases, it will notifies it to the users and when possible help him fix it via a dedicated troubleshoot screen.

Meanwhile, in order to offer a minimal service, and as per Google's recommendation for background activities, quali.chat will launch periodic background sync in order to stays in sync with servers.
...
The F-Droid quali.chat flavor has no dependencies to FCM, therefore cannot relies on Push.
...
That is why on quali.chat F-Droid, the broadcast receiver will acquire a temporary WAKE_LOCK for several seconds (thus securing cpu/network), and launch the service in foreground. The service performs the sync.
