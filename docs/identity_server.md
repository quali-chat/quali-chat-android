...
- The SDK supports incremental sendAttempt (this is not used by quali.chat)
...
- Add email or phone to account (not P1, can be done on quali.chat-Web)
...
- Identity server can be changed on another user's device, so when the change is detected (thanks to account data sync) quali.chat should properly disconnect from a previous identity server (I think it was not the case in Riot-Android, where we keep the token forever)
